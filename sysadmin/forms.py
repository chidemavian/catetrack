...
class resetuserform(forms.Form):
    email = forms.EmailField(label = "Staff E-mail ",max_length = 20,required = True)
    def __init__(self, *args, **kwargs):
...
class creatuserform(forms.Form):
    email = forms.CharField(label = "email",max_length = 20,required = True,widget = forms.TextInput(attrs={'size':'40'}))
...
#     session = forms.CharField(label= 'Session', max_length=13,initial=sess(),widget = forms.TextInput(attrs={'size':'10','readonly':'readonly'}))
#     subclass = forms.ChoiceField(label= 'SubClass',choices=(('---', '---'),('JS', 'JS'),('Science', 'Science'), ('Commercial', 'Commercial'), ('Art', 'Art')))
#     def __init__(self, *args):
