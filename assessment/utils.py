...
##who class statistics


def midtermstat(session,term,klass,arm):
    ...
    for stu in allstu:
        ...
        acaderec.stu_ave1=staver
        acaderec.save()  ## 1. student average

        mid_sum=mid_sum+ casum ###accummulating mid term total
        subject_count=subject_count+totsub  ##accumulating subject count


    try:
        clasAve=mid_sum/int(subject_count)
    except:
        clasAve=0

    StudentAcademicRecord.objects.filter(klass=klass, 
        arm=arm, 
        session =session, 
        term=term).update(class_ave1=clasAve)  ## 2. Class averages 
    ...
    for subject in df:
        ...
        subjectcount.update(subject_avg_mid = subavg)  ## 3. Subject averages
...
def studentaveragemid(admno,term,session,klass,arm): #individual
...
def Annual_statistics(session,klass,arm):
    termm = ['First','Second','Third']

    stuinfo = Student.objects.get(admitted_session = session,
        admitted_class = klass,
        admissionno = admno,
        admitted_arm = arm)
    ...
    for j in stuinfo:
        ...
        [new_subject.append(item) for item in all_my_subject if item not in my_subject] #remove repeatition from list
...
def classroomposition(session,term,klass,arm):
    if term == 'Third':
...
def classposition(session,term,klass):
    if term == 'Third':
        stulist =StudentAcademicRecord.objects.filter(term = term,session = session,klass = klass)
        stuperc = []
        for k in stulist:
