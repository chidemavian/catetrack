...
def wel(request):
    ...
    # k = SubjectScore.objects.fiter(session='2021/2022')
    # for s in k:
    #     s.remarks= s.remark
    #     s.save()
...
def wel2(request): #corect grading 
    ...
    if  "userid" in request.session:
        ...
        k = Student.objects.filter(admitted_session=session1,gone=False,third_term=1)

        for h in k:
            # try:
            di = StudentAcademicRecord.objects.get(student=h,term=term1)
            sdf = SubjectScore.objects.filter(academic_rec=di)
         ...
         ########## Addresssing mid term scores***************8
            ...
            for s in all_students:
                ...
                for subj in all_subjcts:
                    ...
                    mid_term = mysubject.first_ca  +  mysubject.second_ca + mysubject.third_ca
                    mysubject.mid_term_score = mid_term
                    mysubject.save()

                    # msg = mysubject.mid_term_score                  return render_to_response('assessment/selectloan.html',{'msg':msg})
                ...
                if report== 'End term':
                    ...
                    if gh_count == 1:
                        gh_new = gh.get(session=session, 
                            ...
                            arm=arm, 
                            term=term, 
                            reportsheet='Mid term')

                        if gh_new.status == 1:
                            midtermstat(session,term,klass,arm)
...
######......................grade each subject...............................
                ...
                return HttpResponseRedirect('/reportsheet/student/my_results_page/')
...
def my_broad_page(request):
    if 'userid' in request.session:
        varuser=request.session['userid']
        
        if request.method=='POST':
            form = studentbroadform(request.POST)
            if form.is_valid():
                session=form.cleaned_data['session']
                # report = form.cleaned_data['report']
                term= request.POST['term']
                ...
                if term == 'First':
                    ...
                    try:
                        chk=tblresult.objects.get(session=session,reportsheet='End term', term=term)
                    except:
                        varerr= 'Date not set, contact your home room teacher'
                        return render_to_response('assessment/notset.html',{'varerr':varerr})
                    ...
                    return render_to_response('assessment/firstbroad.html',{'varuser':varuser,
                            'school':school,
                            'date':date,
                            # 'varerr':varerr,
                            'replist':replist,
                ...
                if term =='Second':
                    ...
                    try:
                        ...
                        student = Student.objects.get(admitted_session =session,
                            second_term = 1,fullname=varuser,
                            gone = False)

                    except:
                        msg = 'You do not have Second term records'
                        return render_to_response('assessment/selectloan_student.html',{'msg':msg,'user':varuser})
                    ...
                    acaderec = StudentAcademicRecord.objects.get(student = student,
                        term = 'Second', session=session)
                    ...
                    subsco = SubjectScore.objects.filter(academic_rec = acaderec,
                        term = term, session=session).order_by('num')
                    ...
                    try:
                        acaderec1 = StudentAcademicRecord.objects.get(student =student,
                    ...
                    if festrec=='False': #IE FIRST TERM RECORD FOUND
                        try:
                            ...
                            totalmark1 = totalmarkk['end_term_score__sum']
                            totalmark1 = int(totalmark1)
                        except:
                            totalmark1 = 0 #total first term score
                    ...
                    'totalmark2':totalmark2}
                    replist.append(jdic)


                    return render_to_response('assessment/secondbroad.html',{'varuser':varuser,
                            'school':school,
                            'date':date,
                            # 'varerr':varerr,
                            'replist':replist,
                    ...
                    try:
                        student = Student.objects.get(admitted_session =session,
                            third_term = 1,fullname=varuser,
                            gone = False)

                    except:
                        msg = 'You do not have Third term records'
                        return render_to_response('assessment/selectloan_student.html',{'msg':msg,'user':varuser})
...
##OBTAINING FIRST TERM RECORDS *******************
                    ...
                    secrec='False'
                    try:
                        acaderec1 = StudentAcademicRecord.objects.get(student = student,term = 'First',session=session)
                    except:
                        firstrec='True'

##OBTAINING SECOND TERM RECORDS *******************
                    try:
                        acaderec2 = StudentAcademicRecord.objects.get(student = student,term = 'Second',session=session)
                    except:
                        secrec='True'
                    ...
                    if varbeg == 'J' :
                        ...
                        return render_to_response('assessment/reprtthd.html',
                            {
                            # 'varerr':varerr,
                            'replist':replist,'school':school,'term':term,'next':next_term})
                ...
                return HttpResponseRedirect('/reportsheet/student/my_broadsheet/')


        else:
            form = studentbroadform()
            return render_to_response('assessment/broadpage.html',{'varuser':varuser,'form':form})

    else:
        return HttpResponseRedirect('/login/')


def my_results_page_old(request):
    if 'userid' in request.session:
        varuser=request.session['userid']
        if request.method=='POST':
            form = studentresultform(request.POST)
                ...
                if term =='Second':
                    ...
                    if report== 'End term':
                        ...
                        if varbeg == 'J' :
                            ...
                            return render_to_response('assessment/reportsnsecond.html',{'varuser':varuser,
                                    'school':school,
                                    'date':date,
                                    # 'varerr':varerr,
                                    'replist':replist,
                            ...
                            return render_to_response('assessment/reportview_second.html',{'varuser':varuser,
                                    'school':school,
                                    'date':date,
                                    # 'varerr':varerr,
                                    'replist':replist,
    ...
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method=='POST':
                ...
                post= request.POST.copy()
                acccode=post['userid']
                term,session,report = acccode.split(':')
                school=School.objects.get(id =1)
                ...
                try:
                    ...
                    varerr= 'Date not set, contact the admin'
                    return render_to_response('assessment/test.html',{'msg':varerr})
                ...
                if date < chkdate:
                    msg = 'Results are being compiled, kindly check back on'
                    msg2 = chk.deadline
                    return render_to_response('assessment/test.html',{'msg':msg,'msg1':msg2})
                ...
                akrec = StudentAcademicRecord.objects.get(session=session, term =term,student=stuinfo)

                all_subjcts=SubjectScore.objects.filter(academic_rec=akrec,session=session, term=term)

                SUBJECT_COUNT= all_subjcts.count()

                if report == '----':
                    msg = 'select appropriate option'
                    return render_to_response('assessment/ttest.html',{'msg':msg})


                elif report == 'Mid term':

                    msg = 'not Available'
                    return render_to_response('assessment/test.html',{'msg':msg})

                    return render_to_response('assessment/myassessajax.html',{'session':session,'varuser':varuser,'varerr':varerr,'replist':all_subjcts,'term':term})

                elif report == 'End term':

                    return render_to_response('assessment/stuassess.html',{'session':session,
                        'studentinfo':stuinfo,
                        'varuser':varuser,
                        'pos':akrec.position,
                        'varerr':varerr,
...
def editcas2bothtypespp(request,vid):
    if  "userid" in request.session:
        varuser = request.session['userid']
        user = userprofile.objects.get(username = varuser)
        uenter = user.staffname
...
def editcas2bothtypes(request,vid):
    ...
    #s**********************midterm Stats****************
             ...
             ca=classaveragemid(klass1,session1,term1,arm1)#idterm classroom average ******



    #s**********************end term Stats****************

             sa=studentaverageEnd(admno1,term1,session1,klass1,arm1)#*studentaverageEnd*

             

             aklm = SubjectScore.objects.filter(session = session1,
                term = term1,
                klass = klass1,
                arm = arm1,
                subject = subject1)
             ...
             varrid = totsubject['end_term_score__sum']
             subavg = varrid/totstudent
             aklm.update(subject_teacher = uenter.upper(),subject_avg = subavg)##subjectaverageEndterm
                     
             ca=classaverageEnd(klass1,session1,term1,arm1) #*getting end term classroom average**
        
             sp = subjectposition(str(session1),str(subject1),str(term1),str(klass1),str(arm1))#*SubjectPosition***
             cp = classroomposition(str(session1),str(term1),str(klass1),str(arm1))#*Classroom position*
             cp1 = classposition(str(session1),str(term1),str(klass1))#**getting class position
    
             # tn = percent(str(session),str(klass),str(arm),str(admno),str(term))#***calculate percentage
...
def reportsheet(request):
 ...
 ##OBTAINING SECOND TERM RECORDS *******************
                        ...
                        for h in subsco: #subsco are a list of third term subjects
                            if firstrec=='True':
                                fscore = '-'
                            else:
                                try:
                        ...
                        tt=Annual_statistics(session,j.admitted_class,j.admitted_arm)
                    ...
                    if klass[0] == 'S':
                        ...
                        return render_to_response('assessment/reportthirdsss.html',
                            {'varerr':varerr,'replist':replist,'school':school,'term':term,'date':date,'next':ntb})
                    elif klass[0] == 'N' or klass[0] == 'C' or klass[0] == 'L':
                        # if form.cleaned_data['pdffile']:
                        #     template ='assessment/reportnviewthird.html'
                        #     context = {'form':form,'varerr':varerr,'replist':replist,'school':school,'term':term,'stuno':stuno,'classavg':locale.format("%.2f",clavg,grouping=True)}
                        ...
                        return render_to_response('assessment/reportthird.html',
                            {'varerr':varerr,'replist':replist,'date':date,'school':school,'term':term,'next':ntb})
...
def selsubject(request):
    if  "userid" in request.session:
        varuser=request.session['userid']
        staff = userprofile.objects.get(username = varuser,status = "ACTIVE")
     
        if staff.createuser==1: #admin
            form = performanceform()
            return render_to_response('assessment/perform.html',{'varuser':varuser,'form':form})

        ttt= ClassTeacher.objects.filter(teachername=varuser)
        if ttt.count() > 0: #class teacher
            return render_to_response('assessment/success_cl.html',{'varuser':varuser})
    ...
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method == 'POST':
                ...
                post = request.POST.copy()
                acccode = post['userid']
                klass,nm= acccode.split(' ')
                kk = []
                sdic = {}

                data = Subject.objects.filter (klass=klass)
                for j in data:
                    j = j.subject
                    s = {j:j}
                    sdic.update(s)
...
#                 post = request.POST.copy()
#                 acccode = post['userid']
#                 session,klass,room,subject,term,sort = acccode.split(':')

#                 if term == '-----':
#                     msg = 'Select periodicity'
#                     return render_to_response('assessment/test.html',{'msg':msg})
#                 elif sort == '-----':
#                     msg = 'Select how you would like to sort your report'
#                     return render_to_response('assessment/test.html',{'msg':msg})

   
     
#                 if term == 'Annual':
                    # msg = 'Sorry, i am not able to perform this task right now'
                    # return render_to_response('assessment/test.html',{'msg':msg})

                    
#                     if sort == 'stream': #sort = stream 

#                         allst = SubjectScore.objects.filter(session=session,
#                             klass=klass,
#                             subject=subject)#.order_by('annual_avg').reverse()

#                         listcount = allst.count()


#                         midsum =allst.aggregate(Sum('mid_term_score'))
                        
#                         return render_to_response('assessment/streamrep.html',{'session':session,
...
#                         return render_to_response('assessment/subjrep.html',{'session':session,
#                             'replist':allst,
#                             'klass':klass,
#                             'arm':room,
#                             'term':term,
...
#                             term=term,
#                             klass=klass,
#                             subject=subject).order_by('end_term_score').reverse()
...
def seloverall(request):
    if  "userid" in request.session:
        varuser=request.session['userid']
        staff = userprofile.objects.get(username = varuser,status = "ACTIVE")
     
        if staff.createuser==1: #admin
            form = overallperformanceform()
            return render_to_response('assessment/overallperf.html',{'varuser':varuser,'form':form})
...
def getstudentoverallperf(request):
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method == 'POST':
                varuser = request.session['userid']
                ...
                post = request.POST.copy()
                acccode = post['userid']
                session,klass,room,term,sort = acccode.split(':')
                ...
                if term == 'Annual':
                    ...
                    if sort == 'stream': #sort = stream 
                        ...
                        # kp= Student.objects.filter(admitted_session = session,
                        #     admitted_class = klass,gone = False)

                        # for mh in kp:


                        allst = StudentAcademicRecord.objects.filter(session=session,
                            klass=klass).order_by('stu_ave2').reverse()

                        # listcount = allst.count()

                        # allsum =allst.aggregate(Sum('stu_ave2'))
                        # allsum = allsum['stu_ave2__sum']

                        # myscor = allsum / listcount
                        ...
                        return render_to_response('assessment/streamrepover.html',{'session':session,
