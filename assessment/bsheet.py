...
def bsheetforja(term,session,klass,arm,category):
    if klass.startswith('J'):
        k= Subject.objects.filter(category='JS').order_by('num')
    elif klass.startswith('Y'):
        k= Subject.objects.filter(category='Year').order_by('num')
...
def bsheetforsa_original(term,session,klass,arm):
    if arm=='SCIENCE':
        category='Science'
        ...
        for j in stuinfo:
            ...
            for js in sublist:
                ...
                if acaderec_second_count==0:
                    ...
                    try:
                        sss = SubjectScore.objects.get(academic_rec = acaderec_second ,klass = klass,session = session,subject = js)
                        second_termm = {js:sss.end_term_score}                        
                    except:
                        second_termm = {js:0}
